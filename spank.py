...
# --- PARAMÈTRES ---
DOSSIER_AUDIO = "audio/sexy"  # Dossier contenant les sons par défaut
SEUIL_FORCE = 1.5  # Sensibilité (1.0 = gravité normale). Baissez si pas assez sensible.
...
if not fichiers_sons:
    ...
    print("Êtes-vous bien à la racine du dépôt GitHub cloné ?")
    exit(1)
...
print(f"Spank (Version Surface) démarré ! {len(fichiers_sons)} sons chargés.")
print("En attente d'une tape sur l'écran... (Ctrl+C pour quitter)")
...
def on_reading_changed(sender, args):
    ...
    if force > SEUIL_FORCE and (temps_actuel - dernier_choc) > COOLDOWN:
        # Choisir un son au hasard comme le fait le programme original
...
# Réglage de la réactivité du capteur (20 millisecondes)
accel.report_interval = max(accel.minimum_report_interval, 20)

# Démarrer l'écoute des mouvements
